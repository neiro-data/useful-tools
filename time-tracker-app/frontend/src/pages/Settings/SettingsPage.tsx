...
export function SettingsPage(): ReactElement {
  ...
  const canSave = !loading && !saving && form !== null;
  ...
  async function handleCreateCategory(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    // No manual "sort order" input — new categories are appended after the current highest
    // sort_order (or start at 0 when the list is empty).
    const nextSortOrder = categories.length === 0 ? 0 : Math.max(...categories.map((c) => c.sort_order)) + 10;
            ...
            <div className={styles.field}>
              ...
              {timezonesError && (
                <p id="settings-timezone-hint" className={styles.fieldHint}>
                  Couldn&apos;t load the full timezone list; only the current zone is shown. You can still
                  save other settings.
                </p>
              )}
            </div>
          ...
          <form className={styles.categoryForm} onSubmit={handleCreateCategory}>
            ...
            <div className={styles.field}>
              ...
              <p className={styles.fieldHint}>
                Named colours adapt to light and dark mode; a custom hex code always renders exactly as
                picked. Prefer a named colour unless you need an exact hex.
              </p>
