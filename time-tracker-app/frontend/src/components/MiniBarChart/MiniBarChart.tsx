 ...
 * (a month/quarter of days, or weeks) thin the labels so they don't repeat/overlap. Use
 * `barsFromDays`/`barsFromWeeks` (from `./bars`) to build `bars` from report API shapes. */
export function MiniBarChart({ bars }: MiniBarChartProps): ReactElement {
  const max = Math.max(1, ...bars.map((bar) => bar.minutes));
  const isLongRange = bars.length > WEEK_LENGTH;
